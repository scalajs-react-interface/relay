...
object RelayMacro {
  ...
  def relay_macro_impl(c: Context)(code: c.Expr[String]): c.Tree = {
          ...
          .getOrElse({
            ...
            if (!finalDirectory && parent != null) {
              ...
              c.abort(c.enclosingPosition,
                      s"no config found is no directory! ")
          })
      } else {
        c.abort(c.enclosingPosition,
                s"${file.getAbsolutePath} is no directory! ")
      }
    }
    ...
    def copyPath(counter: Int, s: String): String = {
      s.substring(0, s.length - 3) + counter + s.substring(s.length - 3,
                                                           s.length)
    }
    ...
    val inputFilePath =
      Paths.get(rootValue, props.getOrElse("inputFilePath", INVALID_PATH))
    val outputFilePath =
      Paths.get(rootValue, props.getOrElse("outputFilePath", INVALID_PATH))
    ...
    if (!Files.exists(rootPath)) {
      c.abort(
        c.enclosingPosition,
        s"rootPath in ${config.getAbsolutePath}/${config.getName} is not valid ")
    }
    ...
    val out = Source
      .fromFile(outputFilePath.toFile)
      .getLines()
      .toList
      .filter(_.nonEmpty)
    ...
    if (!out.mkString("").contains(START_OF_QUERY)) {
      ...
      c.abort(
        c.enclosingPosition,
        "GraphQL Validation Error please check the console for detailed message.")
    }

    val start = out.zipWithIndex
      .find { case (s, i) => s.contains(START_OF_QUERY) }
      .get
      ._2
    val end =
      out.zipWithIndex.find { case (s, i) => s.contains(END_OF_QUERY) }.get._2
